...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round((df[df['education'].str.contains('Bachelors')].shape[0] * 100 / df.shape[0]), 1)
    ...
    num_high_earn = df.loc[df['salary'] == '>50K', 'native-country'].value_counts()
    num_country = df['native-country'].value_counts()
    highest_earning_country = (num_high_earn / num_country).idxmax()
    highest_earning_country_percentage = round((num_high_earn / num_country).max() * 100, 1)
